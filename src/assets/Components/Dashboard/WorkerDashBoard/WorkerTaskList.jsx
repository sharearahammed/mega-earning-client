...
const WorkerTaskList = () => {
    ...
    useEffect(() => {
      const getData = async () => {
        try {
          const { data } = await axiosSecure.get(`/tasks?page=${currentPage}&size=${itemsPerPage}`);
          setAlltasks(data);
        } catch (error) {
    ...
    useEffect(() => {
      const getData = async () => {
        try {
          const { data } = await axiosSecure.get(`/taskss`);
  ...
  <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    {
        alltasks?.map(task=><WorkerTaskListCard key={task._id}
            task={task}
             />)
    }
    </div>
