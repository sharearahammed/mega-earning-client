...
const FeedBack = () => {
  ...
  <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <ul role="list"
      ...
      {
        feedbacks?.map(feedback=><li key={feedback._id}>
              ...
              <li>
                <figure className="relative rounded-2xl bg-white p-6 shadow-xl shadow-slate-900/10"><svg aria-hidden="true"
                  ...
                  <blockquote className="relative">
                    <p className="text-lg tracking-tight text-slate-900">{feedback.quote}</p>
                  </blockquote>
                  <figcaption className="relative mt-6 flex items-center justify-between border-t border-slate-100 pt-6">
                    <div>
                      <div className="font-display text-base text-slate-900">{feedback.name}</div>
                    </div>
                    <div className="overflow-hidden rounded-full bg-slate-50">
                      <img alt="" className="h-14 w-14 object-cover" src={feedback.photo}/>
                    </div>
                  </figcaption>
                </figure>
              </li>
